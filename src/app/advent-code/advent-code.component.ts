...
@Component({
  selector: 'app-advent-code',
  templateUrl: './advent-code.component.html',
  styleUrls: ['./advent-code.component.scss']
})
export class AdventCodeComponent implements OnInit {
  ...
  daychallenge1(){
    ...
    {
        let firstthreeCount = this.arr[i]+this.arr[i+1]+this.arr[i+2];
        let secondthreeCount = this.arr[i+1]+this.arr[i+2]+this.arr[i+3];
  ...
  daychallenge2(){
    for(let i= 0; i< this.day2Data.length; i++)
    {
         let data = this.day2Data[i].split(' ');
         if(data[0] === 'forward')
         {
          if(this.depth !== 0)
          {
            this.finalValue = this.finalValue+parseInt(data[1])*this.depth;
          }
           this.horizontal = this.horizontal+parseInt(data[1]);
           
         }
         if(data[0] === 'down')
         {
           this.depth = this.depth+parseInt(data[1]);
         }
         if(data[0] === 'up')
         {
          this.depth = this.depth-parseInt(data[1]);
